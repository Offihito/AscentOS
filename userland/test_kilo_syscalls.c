...
void print_num(long num) { printf("%ld", num); }
...
int main(void) {
  ...
  // Create a test file (use /mnt since /tmp dir might not have proper
  // permissions)
  ...
  // Set up a dummy signal handler (won't actually be called without interrupt
  // delivery)
  ...
  long sigaction_ret =
      syscall_rt_sigaction(SIGWINCH, &new_action, &old_action, 8);
  ...
  printf("rt_sigaction(SIGINT, ..., 4) returned: %ld (should be -22)\n",
         sigaction_ret);
  test_assert("rt_sigaction() rejects invalid sigsetsize",
              sigaction_ret == -22);
  ...
  test_assert("TCGETS returns success",
              tcgets_ret == 0 || tcgets_ret == -25 || tcgets_ret == -1);
  ...
  test_assert("TCSETS returns success",
              tcsets_ret == 0 || tcsets_ret == -25 || tcsets_ret == -1);
