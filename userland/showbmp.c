...
int main(int argc, char *argv[]) {
  ...
  for (int y = 0; y < img_h; y++) {
    if (read(fd, row_buf, row_size) != row_size)
      break;

    int screen_y;
    if (is_bottom_up)
      screen_y = start_y + (img_h - 1 - y);
    else
      screen_y = start_y + y;

    if (screen_y < 0 || screen_y >= (int)fb_height)
      continue;
    ...
    for (int x = 0; x < img_w; x++) {
      ...
      if (bytes_per_pixel == 3) {
        color = (0xFF << 24) | (pixel_ptr[2] << 16) | (pixel_ptr[1] << 8) |
                pixel_ptr[0];
      } else if (bytes_per_pixel == 4) {
        color = (pixel_ptr[3] << 24) | (pixel_ptr[2] << 16) |
                (pixel_ptr[1] << 8) | pixel_ptr[0];
      }

      uint32_t *fb_pixel =
          (uint32_t *)(fb_mem + screen_y * fb_pitch + screen_x * 4);
      *fb_pixel = color;
    }
  }
