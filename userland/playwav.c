...
int main(int argc, char *argv[]) {
  ...
  if (fmt.audio_format != 1 ||
    ...
    printf(
        "playwav: unsupported WAV format. Required: PCM, 1/2 ch, 8/16-bit.\n");
  ...
  // Open the audio device - try multiple common paths
  ...
  if (dsp_fd < 0) {
    printf("playwav: could not open any audio device (/dev/dsp, "
           "/dev/hda_audio)\n");
    close(fd);
    return 1;
  }
  ...
  while ((bytes_read = read(fd, buffer, chunk_size)) > 0) {
    ...
    while (total_written < bytes_read) {
      int written =
          write(dsp_fd, buffer + total_written, bytes_read - total_written);
      if (written < 0) {
