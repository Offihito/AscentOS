...
static void I_AscentOS_UpdateSound(void) {
  ...
  if (music.playing && music.sample_buffer) {
    ...
    if (music_samples > 0) {
      int16_t *music_src =
          &music.sample_buffer[music.sample_pos * OUTPUT_CHANNELS];
...
static int16_t synthesize_one_sample(mus_channel_t *ch, int note_idx) {
  if (!ch->notes[note_idx].active || ch->volume == 0)
    return 0;
  ...
  // Envelope: in_release is set immediately by the event parser
  // (MUS_EV_RELEASE_NOTE), so we only need two branches here.
  if (ch->notes[note_idx].in_release) {
    ...
    if (note_age < attack_samples) {
      ...
      envelope = 1.0 - (1.0 - sustain_level) * (note_age - attack_samples) /
                           decay_samples;
    } else {
  ...
  case 15:                       // Sound effects / drums
    if (ch->instrument == 128) { // drum channel
      if (note < 40) {           // bass drum
        ...
        sample = ((double)fast_rand() / 32768.0 - 0.5) *
                 (1.0 - (note_age % 500) / 500.0);
      } else { // hi-hat
...
static int parse_mus_event(void) {
  ...
  case MUS_EV_PLAY_NOTE: {
    ...
    // Find note slot: prefer same note (retrigger), then free, then steal
    // oldest
    int slot = -1;
    int free_slot = -1;
    int oldest_slot = -1;
    ...
    if (slot >= 0) {
      ...
      music.channels[midi_channel].notes[slot].end_tick =
          -1; // -1 = not released
      ...
      music.channels[midi_channel].notes[slot].phase_inc =
          freq / OUTPUT_SAMPLERATE;
    }
    break;
  }
...
static void update_music(void) {
  ...
  while (music.playing && music.sample_count < SAMPLE_BUFFER_SIZE) {
    ...
    synthesize_samples(
        &music.sample_buffer[music.sample_count * OUTPUT_CHANNELS],
...
boolean I_AscentOS_InitMusic(void) {
  ...
  music.sample_buffer =
      (int16_t *)malloc(SAMPLE_BUFFER_SIZE * OUTPUT_CHANNELS * sizeof(int16_t));
...
void I_AscentOS_SetMusicVolume(int volume) { music.volume = volume; }

void I_AscentOS_PauseMusic(void) { music.playing = 0; }
...
void *I_AscentOS_RegisterSong(void *data, int len) {
  if (!data || len < (int)sizeof(mus_header_t))
    return NULL;
...
boolean I_AscentOS_MusicIsPlaying(void) { return music.playing; }
