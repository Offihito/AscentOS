...
// ================================================================
//  TEST ELF BINARY  (293 bytes, generated by tools/gen_test_elf.py)
//
//  Architecture : x86-64, bare-metal (no libc, no syscall)
//  Type         : ET_EXEC  (static executable)
//  Load address : 0x400000
//  Entry point  : 0x400078
//
//  What it does:
//    Writes "Hello from ELF!\r\n" to COM1 (serial port 0x3F8)
//    using busy-wait polling on the TX-empty bit (port 0x3FD),
//    then executes HLT and falls into an infinite loop.
//
//  How to run (from AscentOS shell):
//    elfinfo TEST.ELF      ← dumps ELF header, no load
//    exec    TEST.ELF      ← loads segments, reports entry point
//
//  NOTE: actual execution requires Ring-3 page tables +
//        task_create_from_elf(). The loader maps the binary and
//        reports where it landed; a future task subsystem patch
//        will add the sysret/iret jump to entry.
// ================================================================
/* ── TEST ELF BINARY ─────────────────────────────────────────────────────
 *  Ring-3 syscall test — entry=0x400078, size=259 bytes
 *
 *  Executes in user-mode (Ring-3) via SYSRET:
 *    1. SYS_DEBUG  (8)  — prints "[ELF] Hello from Ring-3!" to serial
 *    2. SYS_GETPID (4)  — reads own PID (discarded)
 *    3. SYS_WRITE  (1)  — writes msg to stdout fd
 *    4. SYS_EXIT   (3)  — clean exit, code=0
 *
 *  No I/O port instructions — fully Ring-3 safe.
 *  All strings are RIP-relative (position-independent within segment).
 * ────────────────────────────────────────────────────────────────────── */
#define TEST_ELF_SIZE 259
static const uint8_t test_elf_binary[TEST_ELF_SIZE] = {
    /* ELF header (64 bytes) — e_entry = 0x400078 */
    ...
    /* Program header (56 bytes) — PT_LOAD, PF_R|PF_X, vaddr=0x400000 */
    0x01, 0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00,  /* p_vaddr = 0x400000 */
    0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x03, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  /* p_filesz = 259 */
    0x03, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  /* p_memsz  = 259 */
    0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    /* .text @ 0x400078 */
    /* mov rax,8 */
    0x48, 0xC7, 0xC0, 0x08, 0x00, 0x00, 0x00,
    /* lea rdi,[rip+0x38] -> msg_debug @ 0x4000BE */
    ...
    /* lea rsi,[rip+0x32] -> msg_hello @ 0x4000D8 */
    0x48, 0x8D, 0x35, 0x32, 0x00, 0x00, 0x00,
    /* mov rdx,43 (msg_hello length) */
    0x48, 0xC7, 0xC2, 0x2B, 0x00, 0x00, 0x00,
    /* syscall  -> SYS_WRITE */
    0x0F, 0x05,
    /* mov rax,3 */
    ...
    /* syscall  -> SYS_EXIT */
    0x0F, 0x05,
    /* hlt ; jmp $ (should never reach here) */
    0xF4, 0xEB, 0xFE,
    /* msg_debug @ 0x4000BE — null-terminated, for SYS_DEBUG */
    0x5B, 0x45, 0x4C, 0x46, 0x5D, 0x20, 0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x20,
    0x66, 0x72, 0x6F, 0x6D, 0x20, 0x52, 0x69, 0x6E, 0x67, 0x2D, 0x33, 0x21,
    0x0A, 0x00,
    /* msg_hello @ 0x4000D8 — "[ELF] SYS_WRITE OK: Ring-3->Ring-0->Ring-3\n" */
    0x5B, 0x45, 0x4C, 0x46, 0x5D, 0x20, 0x53, 0x59, 0x53, 0x5F, 0x57, 0x52,
    0x49, 0x54, 0x45, 0x20, 0x4F, 0x4B, 0x3A, 0x20, 0x52, 0x69, 0x6E, 0x67,
    0x2D, 0x33, 0x2D, 0x3E, 0x52, 0x69, 0x6E, 0x67, 0x2D, 0x30, 0x2D, 0x3E,
    0x52, 0x69, 0x6E, 0x67, 0x2D, 0x33, 0x0A,
};

static const char file_elf_readme[] =
    "test.elf - AscentOS ELF64 test binary\n"
    "  Architecture : x86-64 bare-metal\n"
    "  Type         : ET_EXEC (static executable)\n"
    "  Entry point  : 0x400078\n"
    "  Size         : 293 bytes\n"
    "  Action       : Prints 'Hello from ELF!' to COM1, then halts\n"
    "\n"
    "Usage:\n"
    "  elfinfo TEST.ELF   -- show ELF header\n"
