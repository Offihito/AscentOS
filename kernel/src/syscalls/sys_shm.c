...
int64_t sys_shmget(uint64_t key, uint64_t size, uint64_t shmflg,
    ...
    if (size == 0 || size > SHM_MAX_SIZE) return -22; // EINVAL
...
int64_t sys_shmat(uint64_t shmid, uint64_t shmaddr, uint64_t shmflg,
    ...
    if (shmaddr != 0) {
        ...
        vaddr = vma_find_gap(&t->vmas, aligned_size,
                             SHM_MMAP_REGION_BASE, SHM_MMAP_REGION_LIMIT);
        if (vaddr == 0) {
            spinlock_release(&shm_lock);
            return -12; // ENOMEM
        }
    ...
    if (!(shmflg & SHM_RDONLY)) prot |= 0x2; // PROT_WRITE
...
int64_t sys_shmdt(uint64_t shmaddr, uint64_t a1, uint64_t a2,
    ...
    if (shmaddr == 0 || (shmaddr & (PAGE_SIZE - 1))) return -22; // EINVAL
    ...
    if (!v || !(v->flags & MAP_SHARED)) return -22; // EINVAL
...
void syscall_register_shm(void) {
    ...
    klog_puts("[SHM] Syscalls registered (shmget=29, shmat=30, shmctl=31, shmdt=67).\n");
}
