...
static void mouse_callback(struct registers *regs) {
    ...
    switch (mouse_cycle) {
        ...
        case 2:
            ...
            if (global_mouse_state.x < 0) global_mouse_state.x = 0;
            if (global_mouse_state.y < 0) global_mouse_state.y = 0;
            if (width > 0 && global_mouse_state.x >= (int32_t)width) global_mouse_state.x = width - 1;
            if (height > 0 && global_mouse_state.y >= (int32_t)height) global_mouse_state.y = height - 1;
            break;
    }
}
...
static uint32_t mouse_vfs_read(vfs_node_t *node, uint32_t offset, uint32_t size, uint8_t *buffer) {
    (void)node; (void)offset;
    if (size < sizeof(mouse_device_packet_t)) return 0;

    mouse_device_packet_t packet;
    packet.x = global_mouse_state.x;
    packet.y = global_mouse_state.y;
    packet.buttons = 0;
    if (global_mouse_state.left_button) packet.buttons |= 1;
    if (global_mouse_state.right_button) packet.buttons |= 2;
    if (global_mouse_state.middle_button) packet.buttons |= 4;
