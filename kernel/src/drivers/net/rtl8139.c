...
#define MSR_LINK        (1 << 2)   // Link status (0 = link up, 1 = link down — inverted!)
...
static void rtl8139_irq_handler(struct registers *regs) {
    ...
    if (status & INT_ROK) {
        ...
        while (!(inb(nic_iobase + RTL_CR) & CR_BUFE)) {
            ...
            // Advance the read offset: header(4) + data(rx_length), aligned to 4 bytes
...
void rtl8139_init(void) {
    ...
    ioapic_route_irq((uint8_t)gsi, nic_vector,
                     (uint8_t)lapic_get_id(), irq_flags);
