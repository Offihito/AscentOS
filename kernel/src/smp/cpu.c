...
void ap_main(void) {
    ...
    // 3. Load the IDT for this core
    idt_init();

    // 4. Mark as online to unblock the BSP's boot loop
    ...
    // Enable interrupts now that LAPIC + IDT are ready
...
void cpu_init(void) {
    ...
    if (cpu_count > 1) {
        ...
        memcpy((void*)tramp_virt, trampoline_start, trampoline_end - trampoline_start);

        // Identity-map the 0x8000 page in the active PML4 so the AP safely transitions PAGING -> 64-bit Long Mode
        vmm_map_page(vmm_get_active_pml4(), tramp_phys, tramp_phys, PAGE_FLAG_RW | PAGE_FLAG_PRESENT);
        ...
        volatile uint64_t *ptr_stack = (volatile uint64_t *)(tramp_virt + stack_offset);
        ...
        *ptr_rip = (uint64_t)ap_main;
        ...
        for (uint32_t i = 1; i < cpu_count; i++) {
            if (cpus[i].status == CPU_STATUS_BSP) continue;
