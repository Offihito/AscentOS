...
static int get_height(struct vma *n) {
    return n ? n->height : 0;
}

static uint64_t get_max_end(struct vma *n) {
...
static void update_node(struct vma *n) {
    ...
    if (max_left > n->max_end) n->max_end = max_left;
    if (max_right > n->max_end) n->max_end = max_right;
}
...
static struct vma *insert_node(struct vma *node, struct vma *new_node) {
    ...
    if (new_node->start < node->start)
        ...
        return node; // Duplicate overlapping start boundaries rejected natively inside vma_add earlier
...
static struct vma *delete_node(struct vma *node, uint64_t start, bool *deleted) {
    if (!node) return node;
...
bool vma_remove(struct vma_list *list, uint64_t start, uint64_t end) {
    ...
    while ((v = vma_find_overlap(list, start, end)) != NULL) {
        ...
        // Case 1: Complete overlap - node is entirely subsumed -> do not re-insert anything
...
static struct vma *vma_find_recursive(struct vma *node, uint64_t addr) {
    if (!node) return NULL;
    if (addr >= node->start && addr < node->end) return node;
...
static struct vma *vma_find_overlap_recursive(struct vma *node, uint64_t start, uint64_t end) {
    if (!node) return NULL;
...
static void clone_recursive(struct vma_list *dst, struct vma *node) {
    if (!node) return;
    vma_add(dst, node->start, node->end, node->prot, node->flags, node->fd, node->offset);
...
uint64_t vma_find_gap(struct vma_list *list, uint64_t length, uint64_t base_addr, uint64_t limit_addr) {
    if (list->count == 0) {
        if (base_addr + length <= limit_addr) return base_addr;
        return 0;
    }
    ...
    if (gap_start != 0) return gap_start;
    
    if (current + length <= limit_addr) return current;
