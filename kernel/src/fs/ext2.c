...
static uint32_t ext2_read_impl(vfs_node_t *node, uint32_t offset, uint32_t size, uint8_t *buffer);
static uint32_t ext2_write_impl(vfs_node_t *node, uint32_t offset, uint32_t size, uint8_t *buffer);
...
static int ext2_read_block(ext2_mount_t *mnt, uint32_t block_num, void *buffer) {
    if (block_num == 0) {
...
static int ext2_write_block(ext2_mount_t *mnt, uint32_t block_num, const void *buffer) {
...
static int ext2_read_inode(ext2_mount_t *mnt, uint32_t inode_num, ext2_inode_t *out) {
    if (inode_num == 0) return -1;
    uint32_t group = (inode_num - 1) / mnt->inodes_per_group;
    uint32_t index = (inode_num - 1) % mnt->inodes_per_group;

    if (group >= mnt->groups_count) return -1;
    ...
    if (err) { kfree(block_buf); return -1; }
...
static int ext2_write_inode(ext2_mount_t *mnt, uint32_t inode_num, const ext2_inode_t *inode) {
    if (inode_num == 0) return -1;
    uint32_t group = (inode_num - 1) / mnt->inodes_per_group;
    uint32_t index = (inode_num - 1) % mnt->inodes_per_group;
...
// Allocate a free block from the filesystem. Returns block number or 0 on failure.
static uint32_t ext2_alloc_block(ext2_mount_t *mnt) {
    ...
    for (uint32_t g = 0; g < mnt->groups_count; g++) {
        ...
        if (g == mnt->groups_count - 1) {
            uint32_t remaining = mnt->sb.s_blocks_count - (g * mnt->sb.s_blocks_per_group);
...
static uint32_t ext2_get_block_num(ext2_mount_t *mnt, ext2_inode_t *inode, uint32_t logical_block) {
...
static int ext2_set_block_num(ext2_mount_t *mnt, ext2_inode_t *inode, uint32_t logical_block, uint32_t disk_block) {
    uint32_t ptrs_per_block = mnt->block_size / 4;
    ...
    if (logical_block < ptrs_per_block * ptrs_per_block) {
        ...
        if (dindirect[idx1] == 0) {
            uint32_t new_block = ext2_alloc_block(mnt);
            if (!new_block) { kfree(dindirect); return -1; }
        ...
        uint32_t *indirect = kmalloc(mnt->block_size);
        if (!indirect) { kfree(dindirect); return -1; }
    ...
    if (logical_block < (uint64_t)ptrs_per_block * ptrs_per_block * ptrs_per_block) {
        ...
        if (tindirect[idx1] == 0) {
            uint32_t new_block = ext2_alloc_block(mnt);
            if (!new_block) { kfree(tindirect); return -1; }
        ...
        uint32_t *dindirect = kmalloc(mnt->block_size);
        if (!dindirect) { kfree(tindirect); return -1; }
        ...
        if (dindirect[idx2] == 0) {
            uint32_t new_block = ext2_alloc_block(mnt);
            if (!new_block) { kfree(dindirect); kfree(tindirect); return -1; }
        ...
        uint32_t *indirect = kmalloc(mnt->block_size);
        if (!indirect) { kfree(dindirect); kfree(tindirect); return -1; }
...
static vfs_node_t *ext2_make_vfs_node(ext2_mount_t *mnt, uint32_t inode_num, ext2_inode_t *inode) {
...
static uint32_t ext2_write_impl(vfs_node_t *node, uint32_t offset, uint32_t size, uint8_t *buffer) {
    ...
    while (bytes_written < size) {
        ...
        if (disk_block == 0) {
            ...
            if (disk_block == 0) break; // Out of space
...
static struct dirent *ext2_readdir_impl(vfs_node_t *node, uint32_t index) {
    ...
    ext2_inode_t inode;
    if (ext2_read_inode(mnt, node->inode, &inode)) return NULL;
    ...
    while (byte_pos < dir_size) {
        ...
        if (entry->rec_len == 0) break; // Malformed
...
static vfs_node_t *ext2_finddir_impl(vfs_node_t *node, char *name) {
    ...
    if (ext2_read_inode(mnt, node->inode, &dir_inode)) return NULL;
    ...
    while (byte_pos < dir_size) {
        ...
        if (entry->inode != 0 && entry->name_len == name_len) {
            ...
            if (match) {
                ...
                if (ext2_read_inode(mnt, found_ino, &target_inode)) return NULL;
...
static int ext2_add_dir_entry(ext2_mount_t *mnt, uint32_t dir_inode_num, uint32_t child_inode_num, const char *name, uint8_t file_type) {
    ext2_inode_t dir_inode;
    if (ext2_read_inode(mnt, dir_inode_num, &dir_inode)) return -1;
    ...
    while (byte_pos < dir_size) {
        ...
        if (slack >= needed) {
            ...
            ext2_dirent_t *new_entry = (ext2_dirent_t *)(block_buf + offset_in_block + real_size);
            ...
            uint32_t disk_block = ext2_get_block_num(mnt, &dir_inode, byte_pos / mnt->block_size);
            ext2_write_block(mnt, disk_block, block_buf);
            kfree(block_buf);
            return 0;
        }
    ...
    uint32_t new_block = ext2_alloc_block(mnt);
    if (!new_block) { kfree(block_buf); return -1; }
...
static int ext2_create_impl(vfs_node_t *node, char *name, uint16_t permission) {
    ...
    if (ext2_finddir_impl(node, name) != NULL) return -1;
    ...
    if (ext2_write_inode(mnt, new_ino, &new_inode)) return -1;

    // Add directory entry
    if (ext2_add_dir_entry(mnt, node->inode, new_ino, name, EXT2_FT_REG_FILE)) return -1;
...
static int ext2_mkdir_impl(vfs_node_t *node, char *name, uint16_t permission) {
    ...
    if (ext2_add_dir_entry(mnt, node->inode, new_ino, name, EXT2_FT_DIR)) return -1;
...
static int ext2_readlink_impl(vfs_node_t *node, char *buf, uint32_t size) {
    ...
    ext2_inode_t inode;
    if (ext2_read_inode(mnt, node->inode, &inode)) return -1;
...
static int ext2_symlink_impl(vfs_node_t *node, char *name, char *target) {
    ...
    ext2_inode_t new_inode;
    memset(&new_inode, 0, sizeof(ext2_inode_t));
    new_inode.i_mode = EXT2_S_IFLNK | 0777;  // Symlinks always have 0777 permissions
    ...
    // Add directory entry
    if (ext2_add_dir_entry(mnt, node->inode, new_ino, name, EXT2_FT_SYMLINK)) return -1;
...
static int ext2_remove_dir_entry(ext2_mount_t *mnt, uint32_t dir_inode_num, const char *name);
...
static int ext2_rename_impl(vfs_node_t *node, char *old_name, char *new_name) {
    ...
    // Check if new_name already exists (we don't support overwrite-rename right now)
    ...
    if (ftype == FS_FILE) file_type = EXT2_FT_REG_FILE;
    else if (ftype == FS_DIRECTORY) file_type = EXT2_FT_DIR;
    else if (ftype == FS_SYMLINK) file_type = EXT2_FT_SYMLINK;

    kfree(src_node); // We only needed the info

    // 1. Add the new directory entry
    if (ext2_add_dir_entry(mnt, node->inode, target_inode_num, new_name, file_type)) return -1;

    // 2. Remove the old directory entry
    if (ext2_remove_dir_entry(mnt, node->inode, old_name)) return -1;

    // File rename complete (timestamps inside the moved inode are technically unaffected by rename, 
    // but the parent directory's mtime/ctime will be updated when we add support for that in directory ops)
    return 0;
}
...
static int ext2_chmod_impl(vfs_node_t *node, uint16_t permission) {
    ...
    if (ext2_write_inode(mnt, node->inode, &inode)) return -1;
...
static void ext2_free_indirect(ext2_mount_t *mnt, uint32_t indirect_block) {
    ...
    for (uint32_t i = 0; i < ptrs_per_block; i++) {
        if (ptrs[i]) ext2_free_block(mnt, ptrs[i]);
    }
    kfree(ptrs);
...
static void ext2_free_dindirect(ext2_mount_t *mnt, uint32_t dindirect_block) {
    if (dindirect_block == 0) return;
    uint32_t *ptrs = kmalloc(mnt->block_size);
    if (!ptrs) return;
    ext2_read_block(mnt, dindirect_block, ptrs);
    uint32_t ptrs_per_block = mnt->block_size / 4;
    for (uint32_t i = 0; i < ptrs_per_block; i++) {
        if (ptrs[i]) ext2_free_indirect(mnt, ptrs[i]);
    }
    kfree(ptrs);
...
static void ext2_free_tindirect(ext2_mount_t *mnt, uint32_t tindirect_block) {
    if (tindirect_block == 0) return;
    uint32_t *ptrs = kmalloc(mnt->block_size);
    if (!ptrs) return;
    ext2_read_block(mnt, tindirect_block, ptrs);
    uint32_t ptrs_per_block = mnt->block_size / 4;
    for (uint32_t i = 0; i < ptrs_per_block; i++) {
        if (ptrs[i]) ext2_free_dindirect(mnt, ptrs[i]);
    }
    kfree(ptrs);
...
static int ext2_remove_dir_entry(ext2_mount_t *mnt, uint32_t dir_inode_num, const char *name) {
    ...
    uint32_t dir_size = dir_inode.i_size;
    uint32_t byte_pos = 0;
    uint32_t prev_offset = 0;         // Byte offset of the previous entry in current block
    bool prev_valid = false;          // Whether we have a valid previous entry
    uint32_t current_block_start = 0; // Byte offset where the current block starts
    ...
    while (byte_pos < dir_size) {
        ...
        if (entry->inode != 0 && entry->name_len == name_len) {
            ...
            if (match) {
                ...
                uint32_t disk_block = ext2_get_block_num(mnt, &dir_inode, current_block_start / mnt->block_size);
                ext2_write_block(mnt, disk_block, block_buf);
                kfree(block_buf);
                return 0;
            }
...
static bool ext2_dir_is_empty(ext2_mount_t *mnt, uint32_t inode_num) {
    ext2_inode_t inode;
    if (ext2_read_inode(mnt, inode_num, &inode)) return false;
...
static int ext2_unlink_impl(vfs_node_t *node, char *name) {
    ...
    ext2_inode_t inode;
    if (ext2_read_inode(mnt, target_ino, &inode)) return -1;
    ...
    if (ext2_remove_dir_entry(mnt, node->inode, name)) return -1;
...
static int ext2_rmdir_impl(vfs_node_t *node, char *name) {
    ...
    // Must be empty
    if (!ext2_dir_is_empty(mnt, target_ino)) return -1;
...
int ext2_mount(struct block_device *dev, vfs_node_t *mountpoint) {
    ...
    mnt->inode_size = (mnt->sb.s_rev_level >= 1) ? mnt->sb.s_inode_size : EXT2_GOOD_OLD_INODE_SIZE;
    mnt->groups_count = (mnt->sb.s_blocks_count + mnt->sb.s_blocks_per_group - 1) / mnt->sb.s_blocks_per_group;
    ...
    if (!mnt->bgdt) { kfree(mnt); return -1; }

    for (uint32_t i = 0; i < bgdt_blocks; i++) {
        err = ext2_read_block(mnt, bgdt_block + i, (uint8_t *)mnt->bgdt + i * mnt->block_size);
        if (err) {
